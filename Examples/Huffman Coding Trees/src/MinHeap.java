// C343 
//
// a min-heap implementation, storing elements in an array

public class MinHeap <Key extends Comparable<? super Key>, E> {
    /** store the elements in an array named listArray */
    private BinaryNodeKV<Key, E>[] listArray;
    private int listSize;
    private int maxSize;

    public MinHeap(int maxn, int num, BinaryNodeKV<Key, E>[] pIn) {
        maxSize = maxn;
        listSize = num;
        listArray = pIn;
        heapify();
    ...
    public void insert(BinaryNodeKV<Key, E> pIn) {
    ...
    public BinaryNodeKV<Key, E> removeMin() {
        return remove(0);
    }

    public BinaryNodeKV<Key, E> remove(int pos) {
    ...
    public void swapNodes(int i, int j) {
        BinaryNodeKV<Key, E> tmp = listArray[i];
