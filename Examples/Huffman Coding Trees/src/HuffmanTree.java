...
public class HuffmanTree<Key, E> {
    ...
    private void buildCodeTableHelper(BinaryNodeKV root, String code) {
        codeTable.put((String)root.getValue(), code);

        if (root.getLeft() != null) {
            buildCodeTableHelper(root.getLeft(), code+"0");
        }

        if(root.getRight() != null) {
            buildCodeTableHelper(root.getRight(), code + "1");
        }
